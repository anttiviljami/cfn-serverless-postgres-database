 ...
 */
export const createDatabaseUserWithFullPrivileges = async (props: ResourceProperties) => {
  const database = getSanitizedDatabaseName(props);
...
export const deleteDatabaseUser = async (props: ResourceProperties) => {
  ...
  await query(`revoke all privileges on database ${database} from ${username};`).catch(console.warn);
...
export const getSanitizedDatabaseName = ({ DatabaseName }: ResourceProperties) => {
  return DatabaseName.replace(/[^0-9a-zA-Z_]+/g, '').substr(0, 63);
};

export const getSanitizedUser = ({ DatabaseUser, DatabaseName }: ResourceProperties) => {
...
export const getSecretName = (props: ResourceProperties) =>
  props.SecretName || `rds-credentials/${getSanitizedDatabaseName(props)}/${getSanitizedUser(props)}`;
